...
//             ($file_type=="image/pjpeg")||($file_type=="image/x-png")){} 
  ...
  # Get image width/height to resize properly
     ...
     $img_width = $size[0];
     $img_height = $size[1];
     $height=1040; $width=780; $quality=99;
...
//            }
   ...
   $extension = strrchr($this->request->data['listing_images']['name'],'.');
      ...
      if (!empty($this->request->data['listing_images']['tmp_name'])
    && is_uploaded_file($this->request->data['listing_images']['tmp_name'])
      && ($this->request->data['listing_images']['size']<1000000) && (
            ($this->request->data['listing_images']['type']=="image/gif") 
            || ($this->request->data['listing_images']['type']=="image/png")
              || ($this->request->data['listing_images']['type']=="image/jpeg")
          || ($this->request->data['listing_images']['type']=="image/pjpeg")
	  ||($extension==='.pdf')
          || ($this->request->data['staff_identity']['type']=="image/x-png"))){
...
//check for old document and delete it
   ...
   $file_error = "Unfortunately, we could not upload your image. Image size must be less than 1MB and either of type jpeg, png, gif or x-jpeg or pdf"; 
       ...
       // $images = $this->Properties->Images->find('list', ['limit' => 200]);
        $this->set(compact('property', 'categories', 'states', 'cities', 'agents', 'images',
                'categorysubtypes','category_types'));
        $this->set('_serialize', ['property']);
        $this->viewBuilder()->layout('userbackend'); 
    }
    ...
    public function newproperty()
    {
      // echo $this->request->getData('location');
       //echo "am here...";exit;
        ...
        $agents_table = TableRegistry::get('Agents');
        $agent = $agents_table->find('all')->where(['user_id' =>$this->Auth->user('id')])
                 ->contain(['Subscriptions'=>function($q){return $q->where(['date(enddate) >'=>date('Y-m-d'),'sub_status'=>'active']);}])
                 ->first();
...
if ((empty($size) || ($size[0] === 0) || ($size[1] === 0))) {
    throw new \Exception('this is unacceptable! image must be of type : gif, jpeg, png or jpg and less than 2mb .');
      ...
      if (!empty($this->request->data['listing_images']['tmp_name'])
      && is_uploaded_file($this->request->data['listing_images']['tmp_name'])
      && ($this->request->data['listing_images']['size']<2000000) && (
            ($this->request->data['listing_images']['type']=="image/gif") 
            || ($this->request->data['listing_images']['type']=="image/png")
              || ($this->request->data['listing_images']['type']=="image/jpeg")
          || ($this->request->data['listing_images']['type']=="image/pjpeg")
	  ||($extension==='.pdf')
          || ($this->request->data['listing_images']['type']=="image/x-png"))){
    // encripts the username to be saved as logo name in the db
         $image_name = md5($this->request->data['listing_images']['name']).time(); 
         $extension = strrchr($this->request->data['listing_images']['name'],'.');
         $image_name = $image_name.$extension; 
